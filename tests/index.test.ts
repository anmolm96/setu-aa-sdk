...
import { APIUserAbortError } from 'setu-aa-sdk';
import { Headers } from 'setu-aa-sdk/core';
...
describe('instantiate client', () => {
  ...
  describe('defaultQuery', () => {
    ...
    test('overriding with `undefined`', () => {
      const client = new SetuAaSDK({ baseURL: 'http://localhost:5000/', defaultQuery: { hello: 'world' } });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new SetuAaSDK({ baseURL: 'http://localhost:5000/custom/path/' });
    ...
    afterEach(() => {
      process.env['SETU_AA_SDK_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new SetuAaSDK({ baseURL: 'https://example.com' });
    ...
    test('empty env variable', () => {
      process.env['SETU_AA_SDK_BASE_URL'] = ''; // empty
      const client = new SetuAaSDK({});
      expect(client.baseURL).toEqual('https://fiu-sandbox.setu.co/');
    });

    test('blank env variable', () => {
      process.env['SETU_AA_SDK_BASE_URL'] = '  '; // blank
      const client = new SetuAaSDK({});
      expect(client.baseURL).toEqual('https://fiu-sandbox.setu.co/');
    });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new SetuAaSDK({ maxRetries: 4 });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new SetuAaSDK({});
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new SetuAaSDK({ timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new SetuAaSDK({ fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new SetuAaSDK({ fetch: testFetch });
